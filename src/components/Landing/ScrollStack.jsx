...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = initialCardTopsRef.current[i] ?? getElementOffset(card);
  ...
  useLayoutEffect(() => {
    ...
    // Capture initial, transform-independent offsets to avoid jitter from transform-affecting getBoundingClientRect
    const computeInitialPositions = () => {
      initialCardTopsRef.current = cards.map(el => {
        if (useWindowScroll) {
      ...
      if (endEl) {
        if (useWindowScroll) {
          ...
          endElementTopRef.current = r.top + window.scrollY;
        } else {
    ...
    cards.forEach((card, i) => {
      ...
      // Ensure later cards are above earlier ones when overlapping
      card.style.zIndex = String(1000 + i);
    });

    setupLenis();

    // Ensure image sizes are accounted for before first transform
    ...
    if (pending === 0) {
      ...
      const onImgDone = () => {
        pending -= 1;
        if (pending <= 0) {
          // Recompute anchors after images load
  ...
  // Recompute positions on resize to keep math stable when viewport changes
  useLayoutEffect(() => {
    const handleResize = () => {
      const scroller = scrollerRef.current;
      const cards = Array.from(
      ...
      // Update transforms using new anchors
      updateCardTransforms();
    };
